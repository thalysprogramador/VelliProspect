...
from database import (
    get_all_campaigns as db_get_all,
    create_campaign as db_create,
    insert_lead as db_insert,
    delete_campaign as db_delete,
)
...
def save_campaign(page=None, campaign_data=None):
    ...
    return db_create(
        ...
        region=campaign_data.get("region", ""),
        source=campaign_data.get("source", ""),
        criteria=campaign_data.get("criteria", ""),
        min_score=campaign_data.get("min_score", 7),
        max_results=campaign_data.get("max_results", 100),
    )
